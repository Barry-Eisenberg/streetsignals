    ...
    <div class="signal-card" data-signal-key="${signalKey}">
      ...
      <div class="signal-footer">
        ${signal._storySourceCount > 1
          ? `<span class="signal-source signal-multi-source" data-signal-key="${signalKey}" tabindex="0"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="12" height="12"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>Multiple Sources<span class="signal-multi-source-tooltip">${(signal._storyGroup || []).map(s => { const u = s.source_url || '#'; let d; try { d = u !== '#' ? new URL(u).hostname.replace('www.','') : ''; } catch(e) { d = ''; } d = d || getSignalSourceName(s) || 'Source'; return u !== '#' ? '<a href="' + u + '" target="_blank" rel="noopener noreferrer">' + escapeHtml(d) + '</a>' : '<span>' + escapeHtml(d) + '</span>'; }).join('')}</span></span>`
          : url !== '#'
